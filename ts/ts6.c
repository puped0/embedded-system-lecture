...
void drawing(input ip)
{
	...
	{	
		...
		else if(prev_pressure > 0 & pressure > 0 && newpaint != NULL)		// touching
		{
			...
			{
				int q = (int)((x - paper_pos.x)/8);
				int r = (x - paper_pos.x)%8;	
				char bit = 0x80>>r;
				...
				offset = x + y*db.fbvar.xres;
				*(db.pfbdata+offset) = newpaint->color;
			}	
		}
	}
	else if(drawmode == 4);
...
{					
	if(bit == 0x80)
	{
		newpaint->map[q-1][y-1] |= 0x01;
		newpaint->map[q-1][y] |= 0x01;
		newpaint->map[q-1][y+1] |= 0x01;
	}
	else if(bit == 0x01)
	{
		newpaint->map[q+1][y-1] |= 0x80;
		newpaint->map[q+1][y] |= 0x80;
		newpaint->map[q+1][y+1] |= 0x80;
	}
