...
void drawing(input ip)
{
			...
			if(drawmode == 0)						// line
			{
				...
				if(x>xpos2+2 || x<xpos2-2 || y>ypos2+2 || y<ypos2-2)
				{
					...
					if(xpos2 != xpos1 && xpos2 > 0 && ypos2 > 0)
					{
						...
						printmap(xlow-1, ylow-1, xhigh+1, yhigh+1);	
	...
	else if(drawmode == 4)
	{
		if(prev_pressure==0 && pressure>0)		// on
		{
			xpos1 = x - paper_pos.x;
			xpos2 = y - paper_pos.y;			
			selected = selectpaint(xpos1, xpos2);
			createbg(selected);
			
			printf("select : %d\n", selected);
		}
		else if(prev_pressure>0 && pressure==0)		// off
		{
		...
		else						// touching
		{
			...
			if(x>xpos2+1 || x<xpos2-1 || y>ypos2+1 || y<ypos2-1)
			{
				...
				fx = selected->flag.x;
				fy = selected->flag.y;
				minx = selected->min_pos.x+fx;
				miny = selected->min_pos.y+fy;
				maxx = selected->min_pos.x+fx;
				maxy = selected->max_pos.y+fy;
				...
				if(minx+_x>=0 && maxx+_x<=PAPER_SIZE && miny+_y>=0 && maxy+_y<=PAPER_SIZE)
				{
					printbg(minx, miny, maxx, maxy);
					
			//		selected->flag.x = selected->flag.x + _x;
			//		selected->flag.y = selected->flag.y + _y;
