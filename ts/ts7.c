...
unsigned int bgdot[6][PAPER_SIZE];
unsigned short bgcolor[PAPER_SIZE][PAPER_SIZE];
...
//		printf("x = %d, y = %d, buffer_count = %d\n", ip.x, ip.y, buffer_count);
		...
		if(prev_pressure==0 && ip.pressure > 0)
			printf("on\n");
		else if(prev_pressure > 0 && ip.pressure == 0)
		{	
			printf("drawmode = %d, penmode = %d, current_color = %d\n", drawmode, penmode,current_color);
			printf("off\n");
		}
		*/
...
void drawing(input ip)
{
	...
	const int xpos1, ypos1;
	const int xpos2, ypos2;

	int i, j, k, offset;

	printf("x = %d, y = %d, buffer_count = %d\n", ip.x-64, ip.y-24, buffer_count);
		...
		else if(prev_pressure > 0 & pressure > 0 && newpaint != NULL)		// touching
		{
			...
			else if(drawmode == 3)					// free draw
			{
				...
				for(i=-1; i<2; i++)
				{
					...
					for(j=-1; j<2; j++)
						*(db.pfbdata+offset+x+j) = newpaint->color;
				}				
			}	
		}
	}
...
void dotinmap(char bit, int q, int y) // q is xpos of buffer array (0 ~ 25)
{					
	if(bit == 0x80000000)
	{
		newpaint->map[q-1][y-1] |= 0x00000001;
		newpaint->map[q-1][y] |= 0x00000001;
		newpaint->map[q-1][y+1] |= 0x00000001;
	}
	else if(bit == 0x00000001)
	{
		newpaint->map[q+1][y-1] |= 0x80000000;
		newpaint->map[q+1][y] |= 0x80000000;
		newpaint->map[q+1][y+1] |= 0x80000000;
	}

	newpaint->map[q][y-1] = newpaint->map[q][y-1] | bit | (bit<<1) | (bit>>1);
	newpaint->map[q][y] = newpaint->map[q][y] | bit | (bit<<1) | (bit>>1);
	newpaint->map[q][y+1] = newpaint->map[q][y+1] | bit | (bit<<1) | (bit>>1);
}
...
void createbg(paint* p)
{
	...
	memset(bgdot, 0, sizeof(int)*6*PAPER_SIZE);
	memset(bgcolor, 0, sizeof(char)*48*PAPER_SIZE);
	for(i=0; i<buffer_count; i++)
	{
		if(buffer[i] != p)
		{
