...
void erasemap();
void printframe(const point*, unsigned short);
void printcurrentcolor();
point getbezier(point,point,point,double);
...
const point* draws[7] = {&line_pos, &rect_pos, &oval_pos, &freedraw_pos, &select_pos, &erase_pos, &clear_pos};
const point* pens[2] = {&pen_pos, &fill_pos};
...
{	
	...
	printframe(&freedraw_pos, colors[2]);
	printframe(&pen_pos, colors[2]);
	printcurrentcolor();
}
...
int readcommand(input ip)
{
	...
	if(prev_pressure>0 && pressure==0) // off
	{
...
//			printconsole(newpaint->map);
		...
		else if(prev_pressure > 0 & pressure > 0 && newpaint != NULL)		// touching
		{
			...
			else if(drawmode == 3)					// free draw
			{
				...
				if(_x>=xpos2+1 || _x<=xpos2-1 || _y>=ypos2+1 || _y<=ypos2-1)
				{	
					xpos2 = _x;
					ypos2 = _y;
					...
					if(p1.x > 0  && p1.y > 0)
					{
						...
						{
							result = getbezier(p1, p2, p3, t);

							if(result.x-1<newpaint->min_pos.x)
								newpaint->min_pos.x = result.x-1;

							if(result.y-1<newpaint->min_pos.y)
								newpaint->min_pos.y = result.y-1;

							if(result.x+1>newpaint->max_pos.x)
								newpaint->max_pos.x = result.x+1;

							if(result.y+1>newpaint->max_pos.y)
								newpaint->max_pos.y = result.y+1;
							
							dotinmap(result.x, result.y);

							for(i=-1; i<2; i++)
							{
								offset = (result.y+paper_pos.y+i)*db.fbvar.xres;
								for(j=-1; j<2; j++)
								*(db.pfbdata+offset+result.x+paper_pos.x+j) = colors[newpaint->color];
							}
						}
					}
				}	
				...
				if(x>=xpos2+1 || x<=xpos2-1 || y>=ypos2+1 || y<=ypos2-1)
				{
					...
					if(p3.x > 0  && p3.y > 0)
					{
						...
						for(t=0; t<=1.0; t+=(1.0/10.0))
						{
							...
							if(result.x-2<eraser.min_pos.x)
								eraser.min_pos.x = result.x-2;

							if(result.y-2<eraser.min_pos.y)
								eraser.min_pos.y = result.y-2;

							if(result.x+2>eraser.max_pos.x)
								eraser.max_pos.x = result.x+2;

							if(result.y+2>eraser.max_pos.y)
								eraser.max_pos.y = result.y+2;
							
		
							dotineraser(result.x,result.y);
				
							for(i=-2; i<3; i++)
							{
								offset = (result.y+paper_pos.y+i)*db.fbvar.xres;
								for(j=-2; j<3; j++)
									*(db.pfbdata+offset+result.x+paper_pos.x+j) = colors[0];
							}
						}
					}
...
//		printconsole(buffer[i]->map);
		printf("%d : (%d, %d) ~ (%d, %d) // flag : (%d, %d)\n", buffer[i], buffer[i]->min_pos.x, buffer[i]->min_pos.y, buffer[i]->max_pos.x, buffer[i]->max_pos.y, buffer[i]->flag.x, buffer[i]->flag.y);
		/*********************************/
		free(buffer[i]);
	}
	/**********************************/
	...
	for(i=0; i<buffer_count; i++)
	{
		...
		for(j=ypos1; j<=ypos2; j++)
		{
			for(k=xpos1; k<=xpos2; k++)
			{
				...
				if(bits != 0 && k-fx>0 && j-fy>0 && k-fx<PAPER_SIZE && j-fy < PAPER_SIZE)
				{
					if((bits&bit)==bit)
...
void printframe(const point* p, unsigned short c)
{
	int i, offset;
	int x = p->x;
	int y = p->y;

	offset = (y-2)*db.fbvar.xres;
	for(i=x-2; i<=x+BUTTON_XSIZE+1; i=i+1)
	{
		*(db.pfbdata+offset+i) = c;	
		*(db.pfbdata+offset+db.fbvar.xres+i) = c;
	}

	offset = (y+BUTTON_YSIZE+1)*db.fbvar.xres;
	for(i=x-2; i<=x+BUTTON_XSIZE+1; i=i+1)
	{
		*(db.pfbdata+offset+i) = c;	
		*(db.pfbdata+offset-db.fbvar.xres+i) = c;
	...
	offset = x-2;
	for(i=y-2; i<=y+BUTTON_YSIZE+1; i=i+1)
	{
		*(db.pfbdata+offset+i*db.fbvar.xres) = c;
		*(db.pfbdata+offset+i*db.fbvar.xres+1) = c;
	}

	offset = x+BUTTON_XSIZE+1;
	for(i=y-2; i<=y+BUTTON_YSIZE+1; i=i+1)
	{
		*(db.pfbdata+offset+i*db.fbvar.xres) = c;
		*(db.pfbdata+offset+i*db.fbvar.xres-1) = c;
...
{
	unsigned short bg = makepixel(34,177,76);

	printf("drawmode set : %d\n", cmd);
	printframe(draws[drawmode],bg);
	drawmode = cmd;
	
		
	if(drawmode == 5 && e_selected != NULL)
	{
		int i;
		int fx = e_selected->flag.x;
		int fy = e_selected->flag.y;
		int xpos1 = e_selected->min_pos.x+fx;
		int ypos1 = e_selected->min_pos.y+fy;
		int xpos2 = e_selected->max_pos.x+fx;
		int ypos2 = e_selected->max_pos.y+fy;

		createbg(e_selected);
		printbg(xpos1, ypos1, xpos2, ypos2);

		for(i=0; i<buffer_count; i++)
		{
			if(buffer[i]==e_selected)
				break;
		}
		
		printf("e_selected index : %d\n", i);
	...
	printframe(draws[drawmode], colors[2]);
}
...
void setpenmode(int cmd)
{
	...
	printframe(pens[penmode],bg);
	penmode = cmd - 10;
	printframe(pens[penmode], colors[2]);
}
...
void printcurrentcolor()
{
	...
	for(i=140; i<150; i++)
	{
		offset = i*db.fbvar.xres;
		for(j=280; j<300; j++)
			*(db.pfbdata+offset+j) = colors[current_color];
	}
}

point getbezier(point p1, point p2, point p3, double t)
{
	point result;
	result.x = p3.x*(t*t) + p2.x*(2*t*(1-t)) + p3.x*((1-t)*(1-t));
	result.y = p3.y*(t*t) + p2.y*(2*t*(1-t)) + p3.y*((1-t)*(1-t));
	return result;
}
