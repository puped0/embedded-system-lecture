...
void controlscreen(int cmd, int cursor_pos, int buffer_size, const unsigned char** buffer)
{
		...
		if(db.x - 1 < 0)
		{
			...
			else
			{
				if(scrolled > 0)
				{
					...
					scrolled--;
					for(i=scrolled*MAX_LETTER; i<MAX_LETTER*MAX_LINE + scrolled*MAX_LETTER; i++)
					{
						if(buffer[i] != 0)
							printfont(x_t, y_t, buffer[i]);
						else
							printfont(x_t, y_t, IMG_empty);

						increaseposition(&x_t, &y_t);
					}
					db.x = 9;
				}
			}
	...
	else			// INPUT	
	{
		...
		for(i=cursor_pos; i<buffer_size; i++)
		{		
			printfont(x_t, y_t, buffer[i]);
			increaseposition(&x_t, &y_t);
		}
